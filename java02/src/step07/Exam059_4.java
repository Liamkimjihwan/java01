...
// init() 메서드 정의를 해줘야함.
static void init(Student student, String name, String email, String tel, int age, boolean working)// 파라미터
{ //
  student.name = name;  // 파라미터 명과 인스턴스 명이 같게 해야한다.
  student.email = email;
  student.tel = tel;
  student.age = age;
  student.working = working; // 이 다섯개가 init를 만듦.
}

  // printStudent(arr[0]); 출력을 위한 방법.
  // arr 을 출력하는 것이라면 Student arr 이라고 해야하지만 arr[0] 부분을 출력하는 것이기에
  // student 주소를 받을 student 변수를 선언
static void printStudent(Student student) { // Student 데이터
