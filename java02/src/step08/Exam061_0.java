/* 객체 지향 핵심 문법 - 클래스 정리 : 정리 1 예
=>  정리 1:
   클래스를 바라보는 관점이 클래스는 사용자(개발자)가 프로젝트의 조건에 맞는
   데이터 타입과 그 타입의 데이터를 다루는 연산자를 정의할 때 사용하는 문법
=> 정리 2:
   클래스는 서로 관련된 메서드를 유지보수하기 위해 묶어두는 문법이다.
*/
package step08;
 public class Exam061_0 {
   // 학생의 정보를 다루기 위한 새 데이터 타입 정의
   // 여러 타입의 메모리로 이루어진 복합 데이터
  static class Student { // Student 는 데이터 타입
    // 인스턴스 변수
    // => Student 인스턴스(메모리)를 준비할 떄 만들 메모리 종류
    // 일종의 메모리 설계도
    String name;
    String email;
    String tel;
...
/*
# 생성자
- 아주 특별한 메서드이다. 인스턴스를 생성할 때만 호출할 수 있는 메서드.
- 인스턴스를 생성한 후에는 따로 메서드를 따로 호출할 수 없다.
- 모든 클래스는 생성자를 한 개 이상 가져야한다.
- 만약 생성자가 없으면 컴파일러가 자동으로 기본 생성자를 만든다.
- 생성자를 직접 클래스에 만든다면
  컴파일러는 자동으로 기본 생성자를 만들어주지 않는다.
  -
 생성자의 목적 (존재이유)
  => 메모리(인스턴스/객체)를 사용하기 전에 적절히 유효한 값으로 초기화 시키는 것
  - 모든 클래스는 한 개 이상의 생성자가 반드시 있다.
  = 문법 : 메서드와 유사하다.
  => 클래스명(파라미터 선언, ...) {...}
  생성자는 리턴 타입이 없다.
